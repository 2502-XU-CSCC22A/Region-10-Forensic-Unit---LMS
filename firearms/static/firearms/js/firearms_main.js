...
async function moveToBER(id) {
  ...
  const submitBtn = document.querySelector(
    '#confirmRemovalForm button[type="submit"]',
  );
  if (submitBtn) {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document
    .getElementById("confirmRemovalForm")
...
function updateStats() {
  ...
  const validated = allFirearms.filter(
    (f) => f.validated === "VALIDATED",
  ).length;
  ...
  document.getElementById("issuedBar").style.width =
    Math.max(4, Math.round((validated / (total || 1)) * 100)) + "%";
}
...
function filterTable() {
  ...
  filtered = allFirearms.filter((f) => {
    ...
    const effectiveStatus =
      f.status === "BER" || f.status?.toLowerCase() === "unserviceable"
        ? "Unserviceable"
        : f.status;
...
function buildForm(f = {}) {
  return (
    ...
    inputField(
      "Item Description",
      "f_makeModel",
      f.makeModel !== "N/A" ? f.makeModel || "" : "",
    ) +
    selectField(
      "Status",
      "f_status",
      ["Serviceable", "Unserviceable"],
      f.status || "SERVICEABLE",
    ) +
    selectField(
      "Remarks",
      "f_validated",
