...
async function moveToBER(id) {
    if (!confirm("Move this investigative asset to BER & Disposal?")) return;
    ...
    try {
        ...
        if (invesError) {
            const result2 = await sb
                ...
                .update({ status: "UNSERVICEABLE" })
                .eq("asset_id", id);
            invesError = result2.error;
        }

        if (invesError) {
            console.warn("⚠️ Could not update Investigative_Details:", invesError.message);
        }
        ...
        if (configError) {
            console.warn("⚠️ Config asset update failed:", configError.message);
        }
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    if (addForm) {
        addForm.addEventListener("submit", function(e) {
            const propertyId = document.querySelector("input[name='par_id']").value.trim();
            if (!propertyId) {
...
function autoDismissMessages() {
    ...
    messages.forEach((msg, index) => {
        ...
        if (!msg.querySelector('.close-btn')) {
            ...
            closeBtn.style.cssText = 'float: right; font-size: 20px; cursor: pointer; margin-left: 15px;';
