...
def investigative_view(request):
    ...
    if request.method == "POST":
        ...
        if action == "add":
            ...
            try:
                ...
                InvestigativeDetails.objects.create(
                    asset_id=new_asset, par_id=par_id, office=sub_cat
                )
        ...
        # === UPDATE ITEM ===
            ...
            print(
                f"DEBUG: Update request - Asset ID: {asset_id}, Name: {new_item_name}, Qty: {new_quantity}"
            )
        ...
        # === MOVE TO BER ===
            ...
            try:
                ...
                messages.success(
                    request, f"Asset {asset.model} successfully marked as BER."
                )
            except Exception as e:
...
def par_monitoring_view(request):
    ...
    if request.method == "POST":
        par_number = request.POST.get("par_number", "").strip()
        ...
        if not asset_pk:
            messages.error(request, "Please select an investigative asset.")
            return redirect("InvestigativeEquipment:par_monitoring")

        record = get_object_or_404(InvestigativeDetails, pk=asset_pk)

        record.par_id = par_number
        record.office = issued_to or record.office

        if hasattr(record, "reference_no"):
            record.reference_no = reference_no

        if hasattr(record, "date_issued"):
        ...
        messages.success(request, "PAR record saved successfully.")
        return redirect("InvestigativeEquipment:par_monitoring")

    par_list = (
        InvestigativeDetails.objects.select_related(
            "asset_id", "asset_id__category", "asset_id__status"
        )
        .exclude(asset_id__status__status_name="BER")
        .order_by("-asset_id_id")
    )

    investigative_assets = InvestigativeDetails.objects.select_related(
        "asset_id"
    ).exclude(asset_id__status__status_name="BER")

    for par in par_list:
        expiry = getattr(par, "expiry_date", None)

        if expiry:
            par.days_until_expiry = (expiry - today).days
        else:
    ...
    return render(
        ...
        {
            "par_list": par_list,
            "investigative_assets": investigative_assets,
            "today": today,
            "current_page": "par_monitoring",
            "total_count": par_list.count(),
            "total_comms": all_c.count(),
            "total_firearms": all_f.count(),
            "total_vehicle": visible_v.count(),
            "total_ber": all_d.count(),
...
def ics_monitoring_view(request):
    ...
    ics_list = (
        ...
        .exclude(asset__status__status_name="BER")
        .order_by("-ics_id")
    )

    for ics in ics_list:
        if ics.expiry_date:
            ics.days_until_expiry = (ics.expiry_date - today).days
        else:
            ics.days_until_expiry = 9999

    context = {
        "ics_list": ics_list,
        "par_list": investigative_assets,
        "investigative_assets": investigative_assets,
        "today": today,
        "total_count": ics_list.count(),
...
def move_to_ber_investigative(request, item_id):
    ...
    investigative_records = InvestigativeDetails.objects.filter(asset_id=asset)

    deleted_par = False
    deleted_ics = False

    for record in investigative_records:
        if record.par_id:
            record.par_id = None
            deleted_par = True
            record.save()

    ics_records = ICSRecord.objects.filter(asset=asset)

    if ics_records.exists():
        ics_records.delete()
        deleted_ics = True

    if deleted_par and deleted_ics:
        messages.success(
            request,
            "Investigative asset moved to BER, and PAR and ICS records were deleted.",
        )
    elif deleted_par:
        messages.success(
            request, "Investigative asset moved to BER, and PAR record was deleted."
        )
    elif deleted_ics:
        messages.success(
            request, "Investigative asset moved to BER, and ICS record was deleted."
        )
    else:
