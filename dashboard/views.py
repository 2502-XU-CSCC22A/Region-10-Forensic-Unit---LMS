...
def dashboard_view(request):
    ...
    login_logs = LogEntry.objects.select_related("user").filter(
    ...
    for entry in login_logs:
        ...
        recent_activities.append({
            ...
            # Exact date and time format: e.g., "Oct 24, 2026, 02:30 PM"
            "timestamp": entry.action_time.strftime("%b %d, %Y, %I:%M %p"),
            "unread": entry.id not in read_ids,
        })

    # ==========================================
    # 2. ASSETS WIDGET (CRUD modifications)
    # ==========================================
    # Filters out authentication entries to focus purely on Asset modification records
    asset_logs = LogEntry.objects.select_related("user", "content_type").exclude(
        Q(change_message__icontains="logged in") | Q(change_message__icontains="logged out")
    ).order_by("-action_time")[:5]

    ACTION_FLAG_MAP = {
        ADDITION: "Added",
        CHANGE: "Edited/Moved",  # Django defaults moves or detail updates to CHANGE
        DELETION: "Deleted",
    }
    ...
    for entry in asset_logs:
        ...
        action_text = ACTION_FLAG_MAP.get(entry.action_flag, "Modified")
        
        # Checking if your log messages explicitly capture standard "moved" strings
   ...
   # Query the PAR table directly for records expiring within 30 days
    ...
    ).select_related('asset')  # This pulls the parent config_asset data in one query
    ...
    for vehicle in mobility_expiring:
        pars = vehicle.par_records_set.filter(expiry_date__range=[today, one_month_from_now])
        for par in pars:
            expiring_notifications.append({
    ...
    for comm in comms_expiring:
        pars = comm.communication_par_record_set.filter(expiry_date__range=[today, one_month_from_now])
        for par in pars:
            expiring_notifications.append({
    ...
    for firearm in firearms_expiring:
        pars = firearm.firearms_par_records_set.filter(expiry_date__range=[today, one_month_from_now])
        for par in pars:
            expiring_notifications.append({
    ...
    expiring_notifications.sort(key=lambda x: timezone.datetime.strptime(x['expiry_date'], "%b %d, %Y"))

    # Combined unread count for UI badges
    unread_count = sum(1 for a in recent_activities if a["unread"]) + sum(1 for a in asset_activities if a["unread"])
...
def notifications_api(request):
    ...
    if not category_filter or category_filter == 'investigative':
        for par in InvestigativePARRecord.objects.filter(expiry_date__range=[today, one_month_from_now]).select_related('asset_id'):
            all_expiring.append({
                "category": "Investigative",
                "asset_name": par.asset_id.model if par.asset_id else "Investigative Asset",
                "serial_number": par.asset_id.serial_no if par.asset_id else (par.par_number or "N/A"),
                "expiry_date": par.expiry_date
            })

    # 2. Mobility
    if not category_filter or category_filter == 'mobility':
        for par in Vehicle.objects.filter(expiry_date__range=[today, one_month_from_now]).select_related('vehicle'):
            all_expiring.append({
                "category": "Mobility",
                "asset_name": par.vehicle.make_model if par.vehicle else "Unknown Vehicle",
                "serial_number": par.vehicle.plate_number if par.vehicle else (par.par_number or "N/A"),
                "expiry_date": par.expiry_date
            })

    # 3. Communications
    if not category_filter or category_filter == 'communications':
        for par in Communication.objects.filter(expiry_date__range=[today, one_month_from_now]).select_related('communication'):
            all_expiring.append({
                "category": "Communications",
                "asset_name": par.communication.type if par.communication else "Device",
                "serial_number": par.communication.imei_serial if par.communication else (par.par_number or "N/A"),
                "expiry_date": par.expiry_date
            })

    # 4. Firearms
    if not category_filter or category_filter == 'firearms':
        for par in Firearm.objects.filter(expiry_date__range=[today, one_month_from_now]).select_related('firearm'):
            all_expiring.append({
                "category": "Firearms",
                "asset_name": par.firearm.type if par.firearm else "Weapon",
                "serial_number": getattr(par.firearm, 'faid_serial', par.par_number or "N/A"),
                "expiry_date": par.expiry_date
            })
